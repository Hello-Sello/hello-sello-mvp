#!/usr/bin/env bash
# Run the anon-execute lockdown proof (anon_execute_lockdown_test.sql) against
...
#
# Section (1) is a standing invariant guard: it fails, naming the offenders, if
...
#
# ⚠️  RED-FIRST: section (3) EXITS NON-ZERO against the pre-fix schema by design —
...
TEST_FILE="supabase/tests/anon_execute_lockdown_test.sql"
