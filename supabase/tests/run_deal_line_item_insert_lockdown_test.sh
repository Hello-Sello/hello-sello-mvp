#!/usr/bin/env bash
# deal_line_item direct-INSERT lockdown.
...
echo "== deal_line_item insert lockdown =="
echo "   stack: ${DB_URL}"

out="$(psql "$DB_URL" -v ON_ERROR_STOP=1 < "$HERE/deal_line_item_insert_lockdown_test.sql" 2>&1)" || true
