#!/usr/bin/env bash
# Run the deal-ticket pickup proof (claim_deal_ticket_test.sql) against the
# LOCAL Supabase stack. Prefers host psql; falls back to the stack's DB
# container.
#
# ⚠️  RED-FIRST: EXITS NON-ZERO before the A3 migration lands — the claim
# errors on the missing public.claim_deal_ticket. GREEN once it ships.
set -uo pipefail

TEST_FILE="supabase/tests/claim_deal_ticket_test.sql"
