#!/usr/bin/env bash
# Run the inbox person-RLS proof against the LOCAL Supabase stack (stdin — the
# host psql wrapper mishandles -f). RED-FIRST until PG-5 lands the person branch.
set -uo pipefail
TEST_FILE="supabase/tests/inbox_person_rls_test.sql"
