#!/usr/bin/env bash
# Run the T05 discoverable-shop spec-columns proof
# (discoverable_shop_spec_columns_test.sql) against the LOCAL Supabase stack.
# Prefers host psql; falls back to the stack's DB container. Mirrors
# run_seed_visibility_matrix_test.sh's idiom exactly (single-phase, no race).
set -uo pipefail

TEST_FILE="supabase/tests/discoverable_shop_spec_columns_test.sql"
