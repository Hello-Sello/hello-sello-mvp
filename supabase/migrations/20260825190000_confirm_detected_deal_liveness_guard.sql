-- ============================================================================
-- HEL-74 (closing the gap `critic` found) · confirm_detected_deal gains the
-- SAME relationship-liveness guard as send_deal.
-- ----------------------------------------------------------------------------
-- 20260825180000's own header named send_deal as THE delivery door this pass
-- gates, and scoped create_deal_draft/confirm_deal_change/sign_deal out
-- deliberately — but missed that confirm_detected_deal is a SECOND delivery
-- door: Sella's double-accept path births a card straight into 'negotiation'
-- (D-07, 20260724120400) and explicitly must never call send_deal, so
-- send_deal's guard cannot reach it. A suspended pair's Sella-detected chat
-- suggestion would still deliver a live deal with no check anywhere.
--
-- Re-emits the FULL live confirm_detected_deal body VERBATIM (from
-- 20260724120400_confirm_detected_deal_births_negotiation.sql — the current
-- live definition; NOT any earlier body) with ONE delta: the same liveness
-- check send_deal now runs, placed right before the both-accepted BIRTH block
-- (not at vote-recording) — mirrors send_deal's placement at the moment of
-- actual delivery, not earlier. Note the vote itself does NOT survive a
-- refused birth either: this function persists `chat_message.metadata` (which
-- carries the votes) in ONE statement at the very end, so a RAISE anywhere
-- above it — this new check included — rolls the whole call back, same as an
-- unrelated failure later in the birth path already did before this change.
-- Reactivate, then either side re-accepts, and it births normally.
--
-- Same DROP+CREATE shape as the source (kept for the reason its own header
-- gives: no hard dependency, only the app action calls it).
-- ============================================================================
...
begin
  ...
  if (v_meta->'votes'->>v_ca::text) = 'accept' and (v_meta->'votes'->>v_cb::text) = 'accept' then
    ...
    -- HEL-74: the relationship must still be live at the moment of BIRTH, same
    -- check as send_deal. A RAISE here rolls back this whole call, INCLUDING
    -- the vote recorded above (metadata persists in one statement at the very
    -- end) — see this migration's header. Reactivate, then either side
    -- re-accepts to birth.
    select status into v_rel_status
    from public.relationship
    where id = v_rel and deleted_at is null;
    if v_rel_status is null then
      raise exception 'confirm_detected_deal: relationship not found';
    end if;
    if v_rel_status <> 'active' then
      raise exception 'confirm_detected_deal: relationship is % — no new deals can be born', v_rel_status;
    end if;
