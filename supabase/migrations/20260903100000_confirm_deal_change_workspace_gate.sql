-- ============================================================================
-- confirm_deal_change re-imports the deal-workspace membership check (HEL-85)
-- ----------------------------------------------------------------------------
-- `create or replace`, generated from the LIVE production body (md5
-- 554a25ce2e932fc48cb9edf12f710815, 14655 chars, confirmed byte-identical to
-- local before editing -- .claude/rules/supabase.md: diff against the live body,
-- never a local copy; that is how Discover lost its verified-caller gate).
-- The ONLY edits to that body are the two additive blocks below: one `v_ws`
-- declaration and one guard. No existing predicate, lock order, branch or
-- announcement was touched.
--
-- Signature is unchanged (`p_deal_card_id uuid, p_decision text, p_reason text`
-- RETURNS integer), so `create or replace` is legal here -- no drop/recreate,
-- and the existing grants (postgres, authenticated, service_role) survive
-- untouched rather than needing re-emission.
--
-- ── SEVERITY, HONESTLY: LATENT, NOT LIVE ──
-- Production currently has ZERO private workspaces (all 30 that exist are
-- `company_wide`), so this is not exploitable against production data today.
-- It arms the moment the first workspace is made private, which is a product
-- feature that already exists and is one UPDATE away. Same posture HEL-74 was
-- filed under: fix the latent gap while it is cheap, not after it re-arms.
--
-- ── WHAT AN ATTACKER COULD DO BEFORE THIS ──
-- A company colleague on the relationship, deliberately excluded from a private
-- deal, could resolve its held change: delete the `deal_pending_change` row
-- (silently killing the counterparty's proposal), write `deal_card_log` and
-- `deal_change_input` rows attributed to herself, and land a system-voiced
-- `deal_change_declined` message in a thread RLS hides from her. On the accept
-- arm, with the other side's vote already in, she could commit a new version.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.confirm_deal_change(p_deal_card_id uuid, p_decision text, p_reason text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
declare
  v_uid          uuid := auth.uid();
  v_ws           uuid;   -- HEL-85: the card's live deal_workspace, if it has one
  v_company      uuid;
  v_base         int;
  v_new          int;
  v_votes        jsonb;
...
begin
  ...
  -- ===================================================================
  -- HEL-85: re-import the WORKSPACE half of can_access_workspace.
  -- ===================================================================
  -- The check above is relationship-level. `can_access_workspace` is TWO
  -- conjuncts -- `card_relationship_member(card)` AND (`visibility =
  -- 'company_wide'` OR `is_workspace_member(ws)`) -- and this function
  -- re-imported only the first. A SECURITY DEFINER bypasses RLS entirely, so
  -- nothing downstream re-applies the second.
  --
  -- On a company_wide workspace (the table DEFAULT, and 30 of 33 cards on
  -- production) the two are equivalent, which is why this survived review. On a
  -- PRIVATE workspace the relationship check is strictly weaker: a company
  -- colleague who is not a `deal_member` could resolve the held change, delete
  -- it, write `deal_card_log` + `deal_change_input`, and land a
  -- `deal_change_declined` message in a deal thread she cannot even read. Proven
  -- live before the fix (`confirm_deal_change_workspace_gate_test.sql` §B).
  -- Same defect `announce_deal_event` had (HEL-84 §12); same remedy.
  --
  -- NULL-PASSTHROUGH IS DELIBERATE, NOT AN OVERSIGHT. 3 of 33 production cards
  -- carry no `deal_workspace` row at all (a raw INSERT skips what
  -- `create_deal_draft` normally mints). `can_access_workspace` is EXISTS-based
  -- and would return FALSE for those, so an unconditional call would refuse a
  -- legitimate resolve on every one of them. Where there is no workspace there
  -- is no workspace membership to check, and the relationship check above
  -- remains the operative gate -- exactly the shape
  -- `assert_relationship_writable` uses for a company-less thread.
  --
  -- `uq_deal_workspace_card_active` (UNIQUE on deal_card_id WHERE deleted_at IS
  -- NULL) is what makes this a safe scalar select rather than a multi-row error.
  select w.id into v_ws
  from public.deal_workspace w
  where w.deal_card_id = p_deal_card_id and w.deleted_at is null;

  if v_ws is not null and not public.can_access_workspace(v_ws) then
    raise exception 'confirm_deal_change: caller is not a member of this deal workspace';
  end if;
...
-- Grants re-emitted explicitly. `create or replace` preserves them, so this is
-- belt-and-braces against a future drop/recreate of this file's shape, and it
-- documents the intended reachability in the migration itself.
grant execute on function public.confirm_deal_change(uuid, text, text) to authenticated, service_role;
