-- ============================================================================
-- Phase 3c · confirm_deal_change commits the proposer's note to their card slot (Ayush, 2026-06-18)
-- ----------------------------------------------------------------------------
-- This is a create-or-replace of the FUNCTION BODY ONLY. It SUPERSEDES the
-- 20260617140000_confirm_deal_change_announce.sql body (the only LIVE version
-- of confirm_deal_change at this timestamp), re-emitting it verbatim -- the
-- D-02 two-key gate, the decline branch, the both-accept version-build commit,
-- the line-item snapshot, the private-box carry-forward, the deal_card_log
-- line, the two deal_change_input reason rows, the two Phase 2 announcement
-- inserts, and the delete-on-every-exit -- and adds ONLY two CASE lines inside
-- the both-accept `update public.deal_card` block.
--
-- The note (NOTE-01, D-04): rides the EXISTING held flow. The proposer's note
-- (v_draft->>'note', already carried in the shared draft jsonb -- no new
-- lookup) lands in THEIR slot only on the both-accept commit:
--   - note_company_a  when v_proposer_co = v_ca
--   - note_company_b  when v_proposer_co = v_cb
-- The OTHER side's note column is left UNCHANGED by the else branch (D-02):
-- one side can never overwrite the other's note, because v_proposer_co is
-- structurally one of v_ca/v_cb, never both, and the slot mapping mirrors the
-- canonical company_a_id/company_b_id pair already used for payment_terms_code
-- and the private-box carry-forward -- NOT seller/buyer.
--
-- The announcement inserts are NOT touched: a note-only held change still
-- fires the existing deal_card_updated / deal_change_declined messages (D-09,
-- kept loud); the note text itself is never put into the announcement body.
-- ============================================================================
