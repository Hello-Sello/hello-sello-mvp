...
- **Relationship page** (notes, agreed terms, artifacts) is **in scope** (Relationship page, FR-C6) - shown in the demo (step 3b). It rides on schema-session-7 tables, so it is mostly UI. *(It is off the core deal spine - the deal flow does not depend on it - but it is built for the MVP.)*
...
| 3b | A user opens the **Relationship page** from the chat top bar. | The connected company, status, and any shared notes / agreed terms / artifacts are shown; the other side's private (personal-scope) notes stay hidden. | Connections (FR-C6) |
