...
- **Auth model = Supabase Auth + `person` profile extension.** Identity lives in `auth.users` / `auth.identities` / `auth.sessions` / `auth.mfa_factors` (Supabase-managed). Our `person` table FKs `id` to `auth.users(id)` ON DELETE CASCADE. Email PII handled via mirror pattern: plaintext in `auth.users.email` (Supabase login lookup), pgsodium-encrypted copy in `person.email_encrypted` populated by DB trigger on `auth.users` insert. App code always reads the encrypted column. Mechanism detail (pgsodium / Vault) settled under SCHEMA-DRAFT §A2. *(Locked 2026-05-25, resolves SCHEMA-DRAFT §A1.)*
...
- **Two entry paths into the platform.** Path A (new company) → company row created with `verification_status: pending`, user becomes Superadmin, HS team review queue entry created. Path B (join existing verified company) → join-request entry routed to that company's Superadmin; no new company row, no HS team review for the joining user. At signup: `person.company_id` is nullable until path resolves. *(Locked 2026-05-25.)*
...
- **License files are PII-bearing.** Real business names, addresses, license numbers, responsible-person names. Storage strategy must follow SCHEMA-DRAFT.md PII encryption principle. Build-phase decision: file-storage backend (Supabase Storage / S3) + encryption-at-rest mechanism. *(Locked 2026-05-25.)*
