...
| **Deal Workspace** | The deal container - **Layer B (invited participants only)**. Spawns at Deal Card birth; reached from the Relationship page or a **⤢ button on the Deal Card**. Holds the Deal Chat, the Deal Card (a pinned flip-card pill in the chat), people, THINGS (grouped by domain - the visible work primitive), deal-level documents, and Deal-Sella. Lifecycle **Draft → Confirmed → Done** (Done = delivery note + invoice attached). |
