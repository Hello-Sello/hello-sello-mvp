...
| **Deal Room** *(was "Deal Workspace")* | The deal container. **Born at Deal Card Draft** (the room + deal chat exist from the moment the card is drafted, so negotiation has somewhere to live). **Company-wide visible by default**; `private` collapses it to invited members only. Reached from the Relationship page or a **⤢ button on the Deal Card**. Holds the Deal Chat, the Deal Card (a pinned flip-card pill in the chat), people, THINGS (grouped by the 5-step **Stage** pipeline — the visible work primitive), deal-level documents, and Deal-Sella. Permanent 1:1 with its Deal Card. Lifecycle **Draft → Confirmed → Done** (Done = delivery note + invoice attached). *(Renamed from "Deal Workspace" 2026-06-19, DEV-66. Inline uses of "Deal Workspace" elsewhere in this file still mean this container; the full text + code sweep, and the DB table `deal_workspace` itself, are left for the UI phase.)* |
...
| **Deal Basket** | The reusable "deal-in-assembly": one model + form holding a deal's editable content (products, quantities, prices, delivery, terms, per-side private price, Note) + a **recipient** (company mandatory, person optional). Fed by every trigger (human / Sella / shop); on send it becomes a Deal Card. Transient for now (Option A); persisting it is deferred (Option B). The current `DealForm` is the Basket, unnamed. (2026-06-17, ADR-0003.) |
