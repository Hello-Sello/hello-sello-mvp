...
| **Cloud ledger** | `docs/deploy/cloud-migrations-pending.md` | Entries per slice; app code + migrations are **same-deploy** (RPC shape changes break the old client). ~~⚠️ The ledger currently contradicts itself — the `## PENDING (local only)` header at `:320` is immediately followed at `:322` by `✅ APPLIED 2026-08-16 (Release 2)`.~~ **✅ RECONCILED at T08, 2026-08-23.** *Two corrections to the struck text: those line numbers were stale by ~180 lines even when written (the header sat at `:498`, not `:320`) — cite ledger sections by heading, never by line (L-030). And the defect was larger than "contradicts itself": the section held **13** subsections, **two** carried non-migration deploy debt that survives their applied migrations, and **three** other sections claimed pending work or offered runnable push procedures. All ruled; the ledger now has exactly one section claiming un-pushed migrations, and a separate one for non-migration debt.* (round 1, N13) |
...
| `anon` reads nothing new; every new/redeclared function carries the 3-statement ritual, every recreated **view** re-issues its grants | **Test** — ~~(`anon_execute_lockdown_test.sql`, extended to relations)~~ **CORRECTED at T08, 2026-08-23: `anon_execute_lockdown_test.sql` contains no relation check at all** (0 matches for `view\|relacl\|pg_class\|relkind\|has_table_privilege`); it was never extended. The view half is enforced by **`cross_tenant_lockdown_test.sql:86`** and **`pricelist_item_tier_test.sql:102`**, which both assert `has_table_privilege('anon','public.current_pricelist_item', …)` is false. The function half is enforced by `anon_execute_lockdown_test.sql` as claimed. Recorded as a written criterion on T06 at T08. (round 1, N1) |
