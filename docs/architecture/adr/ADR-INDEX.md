...
| [0010](0010-landing-trust-band.md) | The landing page's new trust section adds no new mechanism — it reuses the page's existing rounded-card idiom instead of going full-bleed, and the repo's existing server-rendered CSS-keyframe pattern instead of a client island, with copy staying inline until a second locale actually exists | landing · public pages · UI · compliance copy · reduced-motion |
