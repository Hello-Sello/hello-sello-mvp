...
**Last updated:** 2026-06-10 11:44 CEST
**Branch:** claude/ayush/work
**Status:** building **3a (deal card)**. Phase 0 DONE (deals module scaffold + pure derive). **Phase 1 IN PROGRESS: about to apply a NEW additive table `deal_party_field` (role-scoped private fields, e.g. seller Margin) to the live DB.**
**Linear issue in progress:** none
**Shared files locked:** ⚠️ **live DB schema** - adding new table `public.deal_party_field` + its own RLS (additive only; no existing table or RLS touched). Releasing as soon as the migration applies.
...
**2026-06-10 (Task 3a - Deal card, Phase 1; ⚠️ NEW table `deal_party_field` - additive, please read).** Starting the deal card. Phase 1 adds ONE new table `public.deal_party_field` to give role-scoped private fields (seller Margin + buyer placeholder, more later from Sell/Buy pages) a home - the schema had none. **One row per (card, version, side, field), RLS `owner_company_id = current_company_id()`** - same privacy spine as `relationship_note`; the other side's app never receives the row. **ADDITIVE only:** new table + its own policy + a seed; **touches none of your existing tables or RLS.** Isolated (only FKs to `deal_card` + `company`), so droppable later with zero blast radius if Sell/Buy design changes it. Will regen `database.types.ts` after. (PO/SO label is derived, gross is computed, version history reads your `deal_card_log` - no other schema changes in 3a.)
