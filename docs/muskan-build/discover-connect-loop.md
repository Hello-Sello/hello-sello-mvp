...
**Status:** 🔨 building — **slices 1–6 built** (catalogue + pricing layered; P12 two-account live walk pending) · **Size:** L · **Owner:** Muskan
...
- **Slice 6 P11 DONE (2026-06-14) — demo catalogue seed.** Migration `20260614170000` (idempotent) gives Aurora a public catalogue — San Raf = L2 (priced), Superseed = L1 (price on request) — so Discover demonstrably shows L0/L1/L2. Records the P8 data as a committed seed (no drift). **P12 (two-account live walk) = user-driven:** all demo companies are connected to Aurora, so the **L1 Request-pricing bar** is walked via a **fresh signup** (unconnected) → Discover → Aurora → Request pricing → Aurora accepts → chat; the **catalogue cards** show from any existing account. Track 1 is **built end-to-end**; awaiting the live visual confirmation.
- **Slice 5 DONE (2026-06-14, P9–P10) — seller `profile_visible` toggle.** Sellers now curate their own openness from the UI (no SQL). **P9** `setProductProfileVisible()` (`catalog/manage.ts`, mirrors `setProductPricePublic`) + surfaced `profile_visible` through `getMyShop`/`ShopProduct` (`catalog/shop.ts`). **P10** two-dial per-product controls in `ShopView` edit mode: an **"On profile / Off profile"** toggle (brand-filled when on) + the existing **"Price public / Price hidden"** toggle, which is **disabled until the product is on-profile** (price visibility is moot when hidden — models the dial nesting). `profile_visible` added to `database.types.ts` **surgically** (not a full regen — avoids pulling in-flight schema, per the team type-edit discipline). tsc clean; eslint clean for the changes (**1 pre-existing `set-state-in-effect` error in the session-18 carousel effect, unrelated — flagged, not fixed**). **Remaining: slice 6 (P11 verified-company seed helper + P12 two-account walk).**
- **Slice 4 P5–P8 DONE (2026-06-14)** — UI + live data verify. **P5** prototyped the L0/L1/L2 + Request-pricing UI (`prototypes/discover-shop-prototype/`) — approved ("like the view, polish later"). **P6** catalogue section on the profile (`[companyId]/page.tsx`): identity + shop fetched in parallel (`Promise.all`), tier chip (Catalogue private / Prices on request / Open catalogue), 2-col `ProductCard` grid (image, specs, price **or** "Price on request"), L0 locked box. **P6a** migration `20260614160000` — recreated `get_discoverable_company` to distinguish connect-state from pricing-state (+ `pricing_requested` field), so requesting pricing no longer makes the **Connect** button lie ("Request sent"). **P7** `RequestPricingActions` — sticky shop-level CTA (one `pricelist_request`, optional note, ~1-day reassurance), state-aware via `pricing_requested`, shown only when a price is hidden **and** not yet connected. **P8** seeded Aurora's catalogue (San Raf = L2 priced, Superseed = L1 price-on-request; applied as data — P11 formalizes); verified via both RPCs as a StonePharm outsider: shop returns L1+L2 with correct gating, `pricing_requested` works. **Note:** every demo company is already connected to Aurora, so the L1 Request-pricing bar surfaces only to an **unconnected** viewer (a fresh signup) — the actual front-door flow. tsc + eslint clean. **Next: live visual walk + slice 5 (seller `profile_visible` toggle) so sellers set visibility via UI, not SQL.**
...
| 4 | ✅ **done** — `product.profile_visible` + RLS + `get_discoverable_shop` → products on profile (L0/L1/L2) + **Request pricing** (`pricelist_request`) | New (data + UI) | — |
| 5 | ✅ **done** — seller `profile_visible` toggle in ShopView (+ price toggle gated on it) | Small | — |
| 6 | ✅ **done (P11)** — demo catalogue seed (`20260614170000`); **P12 two-account live walk = user-driven (pending)** | Test setup | — |
