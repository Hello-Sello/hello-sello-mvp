...
> **Progress 2026-07-24:** ✅ DISC-1..6 (prototype, verified gate, taxonomy, ads banner, CompaniesSection extract, DiscoverShell). ✅ **DISC-7** (`list_discoverable_people` RPC w/ connection_state, pgTAP DP-01..05), ✅ **DISC-8** (people read + mapper, unit 2/2), ✅ **DISC-9/10** (NewPeopleSection cards + person Connect "+", render 3/3) — the People side is live end-to-end (RPC → read → cards → connect). ✅ **DISC-11** (getIncomingConnectionRequests company read, unit 3/3), ✅ **DISC-12** (two-group RequestsSection, render 2/2 — Company via acceptItem/declineItem + People via personActions, wired into shell + page). **NEXT: DISC-13** (getMyConnections server-callable, SHARED connections.ts — sync-lock) → DISC-14 (two-group MyNetworkSection + PG-13 Message button, SHARED ChatView — sync-lock) → DISC-15 (seed) → final verify + loop stop. ⚠️ **Verification note:** DISC-5/6/9/12/14 are interactive client components; the node vitest env has no jsdom, so they're gated by tsc + eslint + `renderToStaticMarkup` smoke tests only. **A live browser pass (or Discover e2e specs) is owed before final ship** — the loop can't drive a browser. Flagging per the prototype-first rule.
