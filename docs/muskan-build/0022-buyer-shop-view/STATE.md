...
ship:   **✅ SHIPPED 2026-08-24** — security round 4 found + closed a live leak (the basket door
        ...
        **▶ ✅ G5 PASSED — Muskan's live walk, RUN 2026-08-24 on production. 10 of 10. Sheet `G5-WALK.md`.**
        **THE SLUG IS COMPLETE. Every gate G1-G5 is closed.** (Bob/StonePharm → Canadian Craft, which is
        the exact criterion-4 fixture — 4 live products, ALL `profile_visible = false`, none
        unfiled). **Three findings raised, none a G5 fail:** F-01 the locked-catalogue panel
        pushes Connect below the fold (Muskan's call: the private/not-connected view must not
        scroll) · F-02 the panel's copy asserts *"connected companies always see the whole
        catalogue"*, falsified twice the same day · F-03 a seller is never told an unfiled
        product is invisible to buyers — direction agreed: ONE *"Not visible to buyers"*
        signal with the reason as subtext, Manage mode only, REPLACING today's "Hidden"
        badge. Both F-01 and F-03 are new UI → prototype before code, sequenced after the walk.
        ✅ **The unfiled-product backfill this file predicted is DONE** — Aurora's 5 products
        refiled to `Berlin` in the UI (2026-08-24), so its shop is no longer empty on prod.
        **CRITERION 6 — THE SLUG'S HEADLINE — PASSED BY EYE ON ITS CARD FRONT, a first:** at G4 it
        only ever passed by regex over the card text. Alice hid `Pedanios 31/1 PND-CA`
        (`profile_visible=false` **and** `price_public=false`); connected Bob still sees the
        product and it reads `Price on request` — **zero € figures, no stepper, no Add to
        basket** — while `COS-CA` on the SAME screen renders `7,50€/g` + tier pills, so the
        price block is proven live, not merely absent. **Card back walked too — criterion 6 is now
        FULLY PASSED, front and back, by eye.**
        The `/connect/inbox` extra check passed by implication (Muskan accepted a request there,
        so the page renders — the crash fix is live). **AC 9 was correctly NOT walked.**
        ⚠️ **Production demo data left modified:** `PND-CA` hidden, `COS-CA` price hidden — both
        deliberate walk fixtures. **Restore before any demo.**
        **Owed after G5:** F-01 + F-03 (prototype-first UI), F-02 (copy), F-04 (Linear ticket that
        must supersede `DECISIONS.md:961` and `:1013` by number), the 3 tier rulings in
        `ROLLUP.md` §C, and Linear for T09-T17. **AC 9 must NOT be walked.**
        ...
        **S8** baseline **80 findings**, none caused by this slug — re-run and diff after the push.
        ...
        `--include-all`. Doc-only, `critic` alone, but a G4 page is now owed (it widens its own
        criteria). Base frozen at `0094f5d`.~~
...
|---|---|---|---|---|
| **T08** | **2 rounds, budget SPENT, did NOT converge — the 8th consecutive ticket. rev 3 folds all 9 blocking findings.** **✅ B-A RULED AND FIXED LIVE, not at the gate** — Muskan: *"amend the file in place"*. `20260607090000`'s third statement granted `execute on functions` to **`anon`**, which `20260817120000` §3 revokes — **live on prod since 2026-08-17**. Locally correct only because the file **replays first**; it is named 2026-06-07 but **authored 2026-08-22** (`d052371`), so on a cloud push it lands **last** and re-widens the default with nothing to narrow it. Fixed (`466cfc2`): `anon` dropped, `authenticated`/`service_role` kept (session 77 left those alone), **replay end state byte-identical** — `f|{postgres=X,authenticated=X,service_role=X}` — and now order-independent. Tables/sequences deliberately untouched (**that arm is T11's; narrowing it here would build T11 inside a documentation migration**). Suites green: anon-execute lockdown, cross-tenant lockdown, T07. **Recorded as L-034 — *a migration's end state on REPLAY is not its end state on PUSH*.** **Two things had made it invisible and both are now rules:** the header's prod check recorded privilege **letters** (*"functions X"*) and **not grantees**, so it structurally could not see who held it; and a back-dated filename makes *"runs first"* read as a property of the file when it is only a property of `db reset`. **rev 2 had explicitly ruled it out** — quoting the migration's own *"verified no-op"* header instead of querying, **L-031's trigger in a plan citing L-030/031/032.** **B-B/C/D/E folded into rev 3's step list:** Phase 13's residue is **two** bolded blocks and rev 2 protected one by line range, excluding the **unique-in-the-file** Cloud-UAT block — *B1's failure mode inside B1's own fix* · Phase 11 carries an **identically-shaped** non-migration block rev 2 never opened (the file flags this class in **three** places) · a **fourth** section (`:478`, *"one batch of 15"*, live commands) still claims pending work and **rev 2's step 5 worsened it** by invalidating its only qualifier · step 6 edited **ADR-0005** with the file in neither `Files`, the deviations table, nor the fence grep — **L-032 and L-017 both cited by the plan and both violated by it** — now added to `Files`, fence-checked, entered as deviation 4. **Deviations now number 5**, incl. the migration amendment ruled live. | **n/a** — doc-only | **0 / 2** — not yet run | **0** — executing, not yet staged |
| ~~T08 rounds 1-2~~ | superseded — **2 rounds, budget SPENT, did NOT converge — the 8th consecutive ticket.** Round 2 = **5 blocking, ALL NEW**, every one inside text rev 2 wrote while fixing round 1. **🔴 B-A — the only finding that changes what lands on production, and it is a conclusion rev 2 WROTE, not a gap it inherited.** `20260607090000_stack_default_privileges.sql:49-50` does `alter default privileges … grant execute on functions to anon, …`; `20260817120000:167-168` **revokes exactly that and has been live on prod since 2026-08-17**. Locally the end state is right — measured: `f|{postgres=X,authenticated=X,service_role=X}`, **`anon` absent** — **but only because `090000` replays FIRST**. Its filename says 2026-06-07; it was **authored 2026-08-22** (`d052371`). **On a cloud push it runs LAST**, `20260817120000` will not re-run, so the grant **re-widens the functions default on production and nothing revokes it again** — re-opening session 77's deny-by-default. Bounded, not nil: the `revoke_anon_execute_on_new_function` event trigger still strips `anon` at `CREATE FUNCTION`, so session 77's *second* mechanism survives — it installed both on purpose. **rev 2 ruled this out** by adopting the migration header's *"a verified no-op"* instead of querying — **L-031's exact trigger, in a plan that cites L-030, L-031 AND L-032.** **B-B:** rev 2 protects Phase 13 by the LINE RANGE `:737-740`, excluding `:742-744` — a separate bolded **⚠️ Cloud UAT required** block (erasure auth-scrub + session-revoke) that is **unique in the file**, with no duplicate at `:794-799`. **B1's failure mode, one bullet block over, inside B1's own fix.** **B-C:** the survey never opened the only OTHER *"Non-migration cloud steps"* block — Phase 11 `:587-589` (`SUPABASE_SECRET_KEY`, `invite.html`); the file flags this class in **three** places and rev 2 found one. **B-D:** a **fourth** section still claims pending work — `:478` *"one batch of 15"* with four live commands — and **step 5 worsens it**: banner-ing `:421-456` invalidates `:480`'s only pointer, leaving `:478-489` the last live-looking push procedure, stale and silent on `--include-all`. **B-E:** step 6 edits a **third** file (ADR-0005) in neither `Files` nor the deviations table — a **Muskan-ruled object** with two G4 fence amendments; **L-032 and L-017 both cited by the plan and both violated by its own step 6.** **Confirmed correct across both rounds, not to be re-litigated:** 13 subsections · **six-file batch** · `20260607090000` unledgered + local-applied (**152 files ↔ 152 rows**, both set-differences empty) · `--include-all` verified against the v2.75.0 binary's own strings · `anon_execute_lockdown_test.sql` has **no** relation check while `cross_tenant_lockdown_test.sql:86` + `pricelist_item_tier_test.sql:102` do · one view in the schema. | **n/a** — doc-only | **0 / 2** — not yet run | **0** — blocked before staging |
| ~~T08 round 1~~ | superseded — **round 1 done → 4 blocking, 5 non-blocking, ALL folded into rev 2. rev 1's CENTRAL CLAIM WAS FALSE.** **B1 — the one that loses information:** rev 1 wrote *"nothing under the NOT-on-cloud-yet header is still pending."* **Phase 13 (`:721`) is** — it owes `supabase functions deploy send-lifecycle-email`, `… erase-expired-accounts` and `supabase secrets set RESEND_API_KEY`, and **`:737-740` is the ONLY step-by-step in the repo for them.** rev 1's step 1, followed literally, files it under APPLIED and destroys those instructions; they survive at `:794-799` only by accident. **The distinction rev 1 lacked: MIGRATION debt and NON-MIGRATION deploy debt are different classes** — a batch can be fully applied and still owe functions and secrets. **B4 — the one that breaks the push:** **the ship batch is SIX migrations, not five.** `20260607090000_stack_default_privileges.sql` appears **zero** times in the ledger, belongs to **no ticket** (which is how it stayed invisible), and is **named 2026-06-07 but authored 2026-08-22** (`d052371`) — so it sorts ~14 months before cloud's tip and **`supabase db push` REFUSES it as out-of-order without `--include-all`** (the class at `:421`). Its own header claims prod application is a verified no-op, so the risk is the push protocol, not wrong grants. **B2:** the section has **13** subsections; rev 1 enumerated **5** — its step 1 would orphan eight. **B3:** a **second** `##` section (`:402`) still shouts **"PRODUCTION IS STILL VULNERABLE"** about `person.company_id`, applied in session 64 five weeks ago (`:247`) — so rev 1's acceptance rule was unachievable. **Notes folded:** rev 1 restated the doc's *"only Phase 10 remains"* instead of the fact (Phase 10 does not remain — **L-031's shape**) · the wider relation sweep rev 1 skipped comes back **CLEAN** (one view in the whole schema, no missed `anon` read) · but **ADR-0005 credits `anon_execute_lockdown_test.sql` as the enforcer and that file has NO relation check** — two other suites do the guarding · the fence grep missed `0005:863`, the mention that most *supports* the plan · and **L-017 was declared in front of nobody** — rev 1 named the widening then routed the gate to `critic` with no gate page, now fixed with a deviations table + an owed `G4-T08.md`. **1 / 2 rounds spent — round 2 recommended: rev 2 rewrote the steps, the inventory and the deviations table, none of it checked, and rev 1's central claim was false.** | **n/a** — doc-only, nothing to test-write | **0 / 2** — not yet run | **0** — not yet staged |
| ~~T08 round 1~~ | superseded — **round 1 running.** Plan rev 1. **The survey already changed the ticket, twice:** (a) criterion 1 says the `PENDING (local only — NOT on cloud yet)` header is followed by *one* `✅ APPLIED` subsection — measured, **all five** subsections under it are applied or done (`:500` Discover ✅ Release 2 · `:528` Allocate ✅ · `:539` Phase 7 ✅ · `:550` DEV-99 ✅ · `:561` the 2026-06-21 batch DONE), so an ops push from that section would **re-push five completed batches**; and the ticket's `:320/:322` citation is stale — the header is at **`:498`**, the file grew ~180 lines (**L-030 again**). (b) criterion 2 has **no hole to close** — T06 re-created `current_pricelist_item` and already issued both grants (`20260822100000:181-182`), verified live: **no `anon` entry in the ACL, `has_table_privilege('anon',…,'SELECT') = f`**. What is missing is that **T06's written criteria never said to do it** — the builder did it from the ADR's grant ritual, so the next view re-creation has no written instruction to inherit. Fix is documentation, marked as such so nobody re-does it. **⚠️ L-017 widening, recorded not assumed:** the missing ledger entries (T05 `20260822090000`, T09 `20260823090000` — **neither has an entry at all**) are **not** in T08's written criteria; three later notes say "T08 owns this", so they are carried here and go on the gate page as a widening. T09's is the one that most needs same-deploy ordering — `store.ts` calls an RPC that does not exist on production. **Fences grepped before adding either file to `Files` (L-032):** neither appears in ADR-0005's `Reused` list, the ADR body's caps, or `STATE.md` § `Locked`. | **n/a** — doc-only, nothing to test-write | **0 / 2** — not yet run | **0** — not yet staged |
| **T07** | **2 rounds, budget SPENT, did NOT converge — the 7th ticket on this slug.** Round 2 = **4 blocking, ALL NEW**, every one in text rev 2 wrote *while fixing round 1* — the slug's signature failure mode, hit again. **B1 (→ Muskan, blocks build):** rev 2 read ADR-0005's fence and concluded *"Fence intact"* **without opening STATE.md's `Locked` entry**, which is stricter — `ShopView` carries **no new state, exactly one new branch**, and that branch is spent (`ShopView.tsx:677-696`). rev 2's remedy proposed state **and** a second branch. The ADR's *"internal handlers"* amendment covers the **catch**, not the **surfacing**. **B2:** `BasketDrawer.tsx:264-267` calls `updateBasketLinePackCount` with no try/catch and drops the Promise — with this policy live a buyer stepping pack count on a now-hidden product gets a **silent no-op + unhandled rejection** (proven live, `UPDATE → 42501`). **DEV-83's shape, 4th instance on this slug.** L-028 fired again: rev 2 swept fixtures by *"Add to basket"*, **not by the verbs the new `WITH CHECK` gates**. **B3:** the component test rev 2 promised **cannot be written** — vitest is `environment: "node"`, no jsdom/testing-library in `package.json`, components render via `renderToStaticMarkup` only, and no `ShopView` test exists at all. L-018's trigger. Replaced with a `42501 → typed error` mapping test the runner can actually run. **B4:** cell 9 **could not detect its own `add-using` mutation** — under it the DELETE succeeds silently affecting zero rows and the buyer's post-delete count also reads 0, so every naive assertion stays green while `rows_left_privileged = 1`. Same class as session 81's four RPC guards with **zero assertions**. Now asserts buyer-visible count *before* and privileged count *after*. **Round 2 also confirmed the core design sound and told me not to churn it** — policy table matches `pg_policy`, EXISTS is RLS-filtered (Eva refused on a `price_public=true` product), cells 1-8/11/12 all behave as claimed, `polqual IS NULL` reproduced, rev 2's corrected grant sentence verified right. Notes folded: 3 mis-cited line ranges · `anon` enumeration completed (`actions.ts:42`) · **`anon` can TRUNCATE this table today — the revoke closes a T11 instance early** · `drop-price-arm` under-listed · cell 10 must assert on the message (grant and RLS refusals share **42501**) · owner + *no price set* cell added · cell 6 must mutate GreenLeaf to `pending` itself · ledger added to Files. **🔴 ADR §7 REPAIRED:** heading `:584` and SQL block `:600` still said `as restrictive for insert` while `:615` said `FOR ALL, not FOR INSERT` — **a builder copying that block ships the ornamental policy rev 6 spent a round removing.** | **0 / 2** — green on the first `test-runner` pass, no retry. Tests-first **RED verified by EXECUTION** (L-023: the test-writer has no Bash and said so — SQL aborted at cell 3, unit 461/1) | **1 / 2** — `critic` 1 blocking + 14 notes, `security` 2 blocking + 4 notes; **one fix pass cleared everything fixable**; 3 findings escalated to G4 rather than fixed (2 fence/scope rulings + the L-017 widening) | **1** — staged 2026-08-23, `G4-T07.md`, **8 items** |
| ~~T07 round 1~~ | superseded — **round 1 → 4 blocking, 9 non-blocking, ALL folded into rev 2.** The checker **installed the proposed policy on the live stack** and ran all 12 cells as real personas in rolled-back transactions — **the core design survived unbroken** (EXISTS-cascade correct in every arm incl. the companyless caller; `WITH CHECK` fires on both the proposed row and the conflict-update row; `polqual IS NULL` confirmed). Defects were all *around* the mechanism: **B1** rev 1 named `actions.ts` as `addToBasket`'s caller — **it is not one** (4 grep hits, none there), and the same paragraph **cited L-031 while violating it**, hours after L-031 was written → recorded as a re-offence under L-031, not a new entry; **B2** `writes.ts` alone **cannot** surface a refusal — `onAddToBasket` returns `void` and `ProductCard.tsx:852` drops the Promise; resolved **inside the ADR fence** by catching one frame earlier in `handleAddToBasket`, which the fence already permits (checked first: `ShopView`'s existing `error` state renders only via `SaveBar`, gated `viewerCanManage && editing`, so it is unreachable for a buyer and cannot be reused); **B3** the grant rationale was **FALSE** — `relacl` carries **no PUBLIC entry**, tables get no default PUBLIC grant, that rule is about `EXECUTE ON FUNCTIONS`; statement kept as defence-in-depth, reason rewritten — **the 4th false claim about DB defaults in this slug's migration prose**, L-010's trigger verbatim; **B4** ten cells, all attacks/refusals, and **no positive cell for the shipped drawer's plain updaters** — the exact thing ADR rev 1 broke (L-011's class). Notes folded: mislabelled "unpriced" rows (both are price-**hidden** with prices set), the `price_public`-without-`pricelist_item` corner now asserted, mutation matrix widened to **4 named `[MUT:]` cells** incl. an `add-using` guard on the shape decision, ledger entry pulled into this build, ADR split + soft-delete owner-arm gap recorded, L-028 fixture sweep done (only live Add-to-basket e2e is owner-arm, survives). **1 / 2 rounds spent — round 2 recommended: rev 2 changed the file set, the client design and the test matrix, none checked by fresh eyes.** | **0 / 2** — not yet run | **0 / 2** — not yet run | **0** — not yet staged |
...
| gate | date | verdict |
|---|---|---|
| **G5 · live walk** | 2026-08-24 | **✅ PASSED — 10 of 10, walked live on production by Muskan. THE SLUG IS COMPLETE.** **PASS 9 — criterion 10 ✅**, walked as the **stale-card** case rather than a synthetic one: Alice turned off `COS-CA`'s public price and Bob's **already-open** tab clicked Add to basket. Server refused **with a visible message and no line written** — so the second conjunct of `product_visible_to_caller AND (owner OR price_public)` is proven live, and **T15's silent-no-op risk did not materialise here.** **PASS 8 — criterion 11 ✅**, swept as Bob **while connected** (the harder arm — connection is what widens every other read): no Save bar, no Manage shop, no banner/logo pencil, no edit affordance anywhere. ⚠️ **And a correction to the F-04 direction, worth more than the tick:** Muskan recalled having decided the Connect-inbox routing before. She had — **in the opposite direction.** `DECISIONS.md:961` locks *"Request-pricing routes to Connect's inbox (type `pricelist_request`)"* and `:1013` locks *"Connect CTAs map to the 4 existing inbox types … no new request types"*. The new direction **reverses both**, so whatever records it must supersede them **by line number** or the next reader will rebuild the inbox route and be correct to. **Linear ticket owed (Muskan: *"maybe you can make a linear ticket. Later"*) — MCP auth still blocked.** **PASS 7 — criterion 3 CLOSED on Muskan's ruling, and F-04 grew from a wording note into a DIRECTION CHANGE.** She accepted Bob's request on Alice's account and ruled the inbox-first flow *"acceptable right now"* and scheduled for replacement: **connection requests move to Discover, pricing requests go straight into the chat, both sequenced with the chat work.** Criterion 3 ticks as shipped. ⚠️ **Constraint recorded against that future design:** the inbox hop IS today's consent step, and *"pricing request goes directly into the chat"* must not let an unconnected buyer put a message in a seller's thread — the exact door T09 closed. The consent has to MOVE, not vanish. **`/connect/inbox` extra check passed by implication** — she accepted a request there, so the page renders and the crash fix is live on production (inferred from the action, not separately confirmed). **PASS 6 — criterion 3's second half opened, and it raised F-04 (wording).** Bob clicked `Request pricing` on `PND-CA` and got `✓ Pricing requested`, **not** a conversation. **Correct as shipped:** the flow is inbox-first — `actions.ts:116` says *"Lands as a `pricelist_request` in their Connect inbox naming the product; accepting runs Connect's existing rollout"* — so the criterion's *"using it opens a conversation"* compresses two steps into one. **A buyer's click MUST NOT open a thread in a seller's messages; that is the consent door T09 closed.** Same class as the `pending` wording ruled at T06's G4 → recommend the same treatment, annotate not rewrite. **Muskan predicted the inbox hop before the code was opened**, which is why this is wording and not a filed failure. Owed: Alice's inbox names the product, and accepting opens the thread. **PASS 5 — criterion 7 ✅ and criterion 6 CLOSED.** Both walked on `PND-CA`, deliberately the **hidden, price-hidden** product rather than an easy one: full spec set (CBG, CBN, terpene %, cultivator, lineage, irradiation, packaging, resealable), **no batch or lot list**, and **no € on the reverse face either** — so the slug's headline invariant is now proven by eye on **both** card faces, where G4 had only a front-face regex. Eye-pass, no screenshot. **PASS 4 — criterion 8 ✅:** moving `COS-CA` off the `500g+` rung re-prices the card **before** the basket is touched (*"yes happening"*). Eye-pass, no screenshot. **PASS 3 — criterion 2 ✅:** `See all prices` on `COS-CA` opens the **full tier ladder**, confirmed by Muskan on screen (*"yes its showing"*). **Recorded as an eye-pass — no screenshot captured**, unlike passes 1 and 2. **PASS 2 — criteria 5 ✅, 6 ⚠️ (front face), 3 ⚠️ (first half), all from ONE toggle.** Alice hid `Pedanios 31/1 PND-CA` — the only Aurora product in the corner that tests the invariant (`profile_visible = false` **and** `price_public = false`) — and connected Bob still sees it (**criterion 5**) reading `Price on request` with **zero € figures, no quantity stepper, no Add to basket** (**criterion 6, the slug's headline, seen by a human for the first time** — at G4 it passed only by regex over the card's `innerText`). **The control was on the same screen, which is what makes this stronger than the G4 run:** `COS-CA` renders `7,50€/g`, a struck-through `8,00€`, tier pills `500g+/1000g/1200g+` and Add to basket, so criterion 6 cannot be passing because prices are broken. `10/10 MBE-CA` (visible, price hidden) gave **criterion 3's first half** — `Price on request`, no stepper, no Add to basket, its own `Request pricing` button; the click and the conversation naming the product are **not** walked. Confirmed incidentally, not ticked: **no "Hidden" badge on Bob's copy of the hidden product** — the buyer mapper genuinely does not carry `profile_visible`, rather than the badge being styled away — and no owner chrome on the grid (partial criterion 11). **Muskan reached the correct reading unprompted** — *"it is because I am connected right?"* ⚠️ **A planned route was found blocked and abandoned:** criterion 5 was first staged against Canadian Craft, whose only user is **Marcel himself** (`marcel@canadiancraftco.de`) — nobody walkable can accept the connection, and sending one would put a live request in the customer's inbox. Aurora carried it instead. Criteria **1** and **4** PASSED in one pass (Bob/StonePharm → Canadian Craft: banner, Verified pill, info/location/links boxes all render; **no** Manage shop, Save bar or banner/logo pencil anywhere; locked panel + Connect, zero products). **Canadian Craft turned out to be the exact criterion-4 fixture** — 8 products, 4 soft-deleted, and **all 4 live ones `profile_visible = false`**, none unfiled, so "hidden every product" is literally true of it rather than approximated. **Three findings, none failing its criterion — F-01/F-02/F-03, full text in `G5-WALK.md` § Findings.** F-01 is the one Muskan ruled on live: the locked-catalogue panel's `p-12` padding + 🔒 tile + 3-line copy + optional-note textarea push **Connect off-screen at 1440**, so the buyer reads *"Connect with them"* with no control in view — **the same defect AC 4 was written to prevent, and the second time this slug has hit it** (`LockedCatalogue`'s own header records the first: moving the action INTO the panel fixed which component owns it, not whether it is on screen). Ruling: the private / not-connected view must not scroll. **F-02:** the panel's copy asserts *"connected companies always see the whole catalogue"* — falsified twice on 2026-08-24 (Bob was already connected to Aurora and still got the panel; an unfiled product is withheld from a connected buyer too). The true rule is *connection reveals HIDDEN products, not unfiled ones, and never a price.* **F-03:** three independent reasons make a product invisible to buyers and the seller is told about exactly one — `profile_visible=false` gets a badge, `location is null` and the visibility window get nothing; **this is what made Aurora's shop read as broken** to its own owner. Direction agreed: ONE *"Not visible to buyers"* signal, reason as subtext, **Manage mode only**, replacing the "Hidden" badge rather than sitting beside it (which also settles Muskan's objection that "Hidden" renders in the seller's **shop** view — `ProductCard.tsx:546` gates on `profile_visible === false && !editing`). **✅ The backfill § *Owed* predicted is DONE:** Aurora's 5 unfiled products refiled to `Berlin` in the UI, so the *"Aurora's shop goes empty on production"* warning from T05's G4 is **discharged**. StonePharm's 3 remain unfiled. **Still owed: criteria 2, 3, 5-8, 10, 11, and the `/connect/inbox` extra check. AC 9 must NOT be walked.** |
| **G4 · T08** | 2026-08-23 | **PASSED — Muskan: *"fine, pass it"*. All 8 deviations ACCEPTED** — the scope beyond T08's two written criteria was ruled sound, the reasoning being that the ledger was unusable for the push without it and one item was a section **falsely telling ops production was still vulnerable**. Page `G4-T08.md`; page `G4-T08.md`.** **A doc-only XS ticket found a PRODUCTION SECURITY REGRESSION in the ship batch** — `20260607090000` granted `execute on functions` to `anon`, which `20260817120000` §3 revokes and which has been **live on prod since 2026-08-17**; correct locally only because the file **replays first** (named 2026-06-07, **authored 2026-08-22**), so on a cloud push it lands **last** and re-widens the default with nothing to narrow it. **Muskan ruled it LIVE, not at the gate** — *"amend the file in place"* (`466cfc2`); replay end state **byte-identical**, now order-independent. **L-034.** **The ledger is now executable:** it said *"push the slug as one batch, in timestamp order"* and **could not be executed from itself** — 3 of 6 migrations had no entry. Now: `## PENDING (local only)` → `## ✅ HISTORICAL` with all **13** subsections ruled · a new `## ⚠️ OUTSTANDING — NON-MIGRATION DEPLOY DEBT` heading (**migration debt and deploy debt are different classes** — a batch can be fully applied and still owe edge functions, secrets and UAT) · the section shouting *"PRODUCTION IS STILL VULNERABLE"* about a session-64 fix **struck, not deleted** · **both** stale push procedures bannered `SUPERSEDED` · all six entries written, `20260607090000`'s carrying `--include-all` and a pre-flight that **names the ROLES** (a letters-only check is what hid L-034). **Acceptance rule verified independently, not asserted.** **⚠️ THE BUILDER RAN THE RULE AND IT FAILED FIRST PASS — nine more stale claims none of the three plan revisions listed**, incl. **two `####` headings still ending "— NOT on cloud"**, a Phase-10 row *"Deferred — Push when it ships"*, and three Phase-7 siblings called *"still local-only"* that are all live. **A reader who jumps to a heading never sees the section banner.** Fixed in place. **Checker: 2/2 spent, did NOT converge — 8th consecutive ticket; 9 blocking across the two rounds, round 2's five ALL NEW and all inside round 1's own fixes.** Gate: clean reset · **39/39 runners over 44 suite files** (5 never execute). **Phase 11's `invite.html` kept as OWED, not swept** — the 2026-06-23 entry says *"3 email templates"* and never names which. |
| **G4 · T07** | 2026-08-23 | **PASSED — all 8 items ruled.** The final four (2026-08-23): the **L-017 widening KEPT** — the refusal message covers the basket quantity stepper as well as Add, because the `FOR ALL` policy gates both and mapping only one would ship the raw `new row violates row-level security policy` text as the expected message for a flow the ticket calls routine · the **seed-pollution class → its own ticket, T12** (*"fix it properly"*; not urgent, does not block `/ship`) · the **test-file fence stays ABSOLUTE** — findings in test files always route to `test-writer`, even on an explicit orchestrator instruction, because the fence is about capability not intent and *"only edits tests when told to"* is not a safeguard when the telling is routine (**L-035**) · the **refusal bar ACCEPTED as built** despite skipping G2 (the prototype has no refusal state to compare against) — one message in a rare situation, seen in `g4/28h`, reads clearly and fits at 900. **Original 3 of 8, all BUILT and proven:** **Ruled:** (1) `BasketDrawer`'s crossing of ADR-0005's `Reused` fence → **amend** (written into `0005:828` + `Locked`, with the WHY and how it was missed — **L-032**); (2) `authenticated` holding TRUNCATE → **close it here, not T11** — named-verb revoke, ACL now `authenticated=arwd/postgres` with `anon` absent entirely, **red-proved by re-granting**; (3) the stale card → **keep the message, add the refetch** — Muskan: *"we can show no longer available"*; researched first and the industry pattern is the client refetches rather than leaving the user to discover a reload is needed. *"Try again"* was rejected on the same evidence: it promises a transient failure, and this is permanent for that buyer. **e2e red-proves the fix** (remove `router.refresh()` → `toHaveCount(0)` resolves to 1 element, the forever-loop). **Still owed:** the L-017 widening to the two updaters · the seed-pollution class (2 shipped suites affected — *"39/39 green" is only true on a fresh reset*) · the test-file fence · the pill having skipped G2 (the prototype has **no** refusal state to compare against). **Gate:** T07 suite PASSED · **39/39 runners over 44 files** · unit **462/462** · `tsc` 0 · eslint 6 pre-existing · e2e **131 pass / 21 fail / 152** (the 21 = the two recorded classes; total moved 151→152 by the one new test) · **4 mutation cells each redding their documented cells**. `security` ran S1–S5 and **could not get through** — the decisive probe was a Bavaria product with `profile_visible=false` but `price_public=TRUE`: if the EXISTS were not RLS-filtered the price arm alone would have admitted it. Refused. **S6+S8 owed at `/ship`.** **Both migration-header claims about DB defaults verified TRUE — the four-false-claims streak is broken.** **Two builder judgement calls worth keeping:** it **refused to edit `supabase/tests/**` itself** (its fence) and dispatched `test-writer` rather than editing or ignoring the ruling; and, finding vitest genuinely cannot express a refetch (node env, no jsdom, no `ShopView` test in the tree), it put the assertion in Playwright **with a fixture it creates and destroys**, rather than writing a weaker unit test that looks like coverage. |
| **G4 · T06** | 2026-08-23 | **PASSED — Muskan: *"pass"*. All 6 items ruled in one word: the location filter **shape** ACCEPTED as shipped (dropdown + grouped bands, not the prototype's flat 4-up under pill tabs); criterion 5's `pending` wording ACCEPTED and annotated rather than rewritten (see the amendment in `TICKETS.md` — the schema holds no pending relationship, the behaviour is right, the sentence invited a future wrong "fix"); the missing **"why the catalogue grew"** signal ACCEPTED as a deliberate non-build — the prototype's `6 products · full catalogue (connected)` counter does **not** ship, and this row is why, so it is not re-found later as an oversight; the seed `region`/`location` disagreement on AUR-1D ACCEPTED as seed data (**fix before any demo** — it renders `Region · Toronto` under a `Montreal Warehouse` header, and T06 is what first puts the two on one screen); `anon`'s remaining INSERT/UPDATE/DELETE/TRUNCATE on `product_media` FOLDED INTO T11 (criterion 9 asked only for SELECT and got it — not a T06 defect); and the vanished G2 visual contract ACCEPTED as recorded. Staged page `G4-T06.md`, 22 shots `g4/14`–`26b`. Staging detail follows. **6 items were staged; page `G4-T06.md`, 22 shots `g4/14`–`26b`.** **AC 6 is the ticket and it PASSES:** AUR-1D (`profile_visible=false` **and** `price_public=false`) is revealed to connected Bob as `Price on request` with **zero** currency figures on either card face — asserted by regex over the card's whole `innerText`, not by eye. Structural, not incidental: the override attaches to `profile_visible` only, and `price_public` stays a separate un-overridable conjunct at all three layers (policy · view `AND p.price_public` · RPC `case when p.price_public then …`). Matrix walked in full — AUR-1C (hidden, priced) reveals **with** `4,00€/g`; AUR-1A (visible, unpriced) stays priceless; AUR-1B control renders `6,00€/g`, proving the price block works. Controls both hit: **Eva 4 products / one location / no dropdown; Bob 6 / two locations / dropdown `All 6 · Montreal 2 · Toronto 4`.** No horizontal overflow at 1440 or 900. **Re-verified on current HEAD, not trusted from 2026-08-22** — T09 shipped after T06 built and revoked the `relationship` INSERT grant, so T06's fixtures were re-run against it: **not broken, no fixture needed to move.** Gate: clean reset · **38/38 SQL runners over 43 suite files** (5 never execute) · `tsc` 0 · eslint 6 all pre-existing · unit **458/458** · targeted e2e **13/13** · full e2e **113 pass / 21 fail / 17 skipped of 151**, all 21 classified, three A/B-proven against `d96f68e`. **⚠️ e2e baseline corrected: 151 total, not 146** — the 21 failures and both classes are unchanged; specs were added since (T06's own positive `location-menu-btn` test among them). **⚠️ A polluted measurement was caught and discarded, not reported:** the first full-suite run followed a targeted run and one deal-ticket test failed from leftover state; on a clean reset it passed — the F-05 persistence trap, hit again. **Criterion 10 marked CANNOT-VERIFY, not passed:** the outcome matches live `polqual`, but that the site-1 body was *diffed* rather than re-typed is not observable after the fact. **6 items:** (1) location filter **shape** — live ships a dropdown + grouped bands, the prototype had a flat 4-up grid under pill tabs; presence was ruled at T05, shape never was; (2) criterion 5 names a `pending` state `relationship_status` cannot hold (`active`/`ended`/`suspended` only) — behaviour correct via Eva's pending inbox item, the wording invites a future "fix"; (3) the buyer is never told **why** the catalogue grew — the prototype's `6 products · full catalogue (connected)` counter didn't ship; (4) seed `region`/`location` disagree on AUR-1D (`Region · Toronto` under a `Montreal Warehouse` header) — T06 is what first puts them on one screen; (5) `anon` keeps INSERT/UPDATE/DELETE/TRUNCATE on `product_media` — criterion 9 asked only for SELECT and got it, T11's class; (6) the G2 visual contract no longer exists (the in-app route was deleted at `/build`, as designed), so shape rows are judged against HTML variant A, which its own NOTES call insufficient. **Three shipped-comment errors FIXED today** (`00c22b3`, comment-only — 0 non-comment lines changed in the migration): TRUNCATE is gated by the table grant not RLS · the cascade list is **three** not four (`plit_public_select` self-gates) · ADR-0005 row 4 `untouched` → role list narrowed, and `:298-301`'s view claim corrected. **⚠️ STATE.md's own claim that "the ledger is right" was WRONG** — `cloud-migrations-pending.md:67` repeated the same over-count and nobody had opened it; corrected in the same pass, recorded as **L-031**. **Owed before `/ship`, not gate items:** the ledger still lacks T05's `20260822090000` and T09's `20260823090000` (T08 owns it). |
