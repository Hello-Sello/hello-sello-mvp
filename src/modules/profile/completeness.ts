...
// so single-name (mononym) and social-login identities can complete onboarding. The
// profile photo is OPTIONAL (DEV-99 #4) — display name + title are the only gates.
...
export function isProfileComplete(p: ProfileCompletenessInput): boolean {
  return Boolean(p.displayName?.trim() && p.title?.trim())
}
