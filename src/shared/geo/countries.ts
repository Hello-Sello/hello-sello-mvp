/**
 * Canonical ISO-3166-1 alpha-2 country list (code → English short name).
 *
 * Single source of truth for country display + the Discover country filter.
 * `company.country` is stored CHAR(2) ISO alpha-2, so these codes line up
 * directly. This REPLACES the 14-entry hand-kept `COUNTRY_NAMES` that used to
 * live inline in `src/app/discover/companies.ts` (D-14 + DRY: one list, not two).
 *
 * The full list is intentional: the Discover country filter is driven off the
 * canonical ISO set (every country selectable), NOT off whatever countries
 * happen to be present in the fetched directory rows.
 */
...
/** Display name for an ISO alpha-2 code; falls back to the raw code if unknown. */
export const countryName = (code: string): string => COUNTRIES[code?.toUpperCase()] ?? code;
